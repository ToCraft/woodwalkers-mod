...
public class CantInteractTrait<E extends LivingEntity> extends ShapeTrait<E> {
    ...
    public static final Codec<CantInteractTrait<?>> CODEC = RecordCodecBuilder.create((instance) -> instance.group(
            Codec.BOOL.optionalFieldOf("can_only_interact_with_listed", false).forGetter(o -> o.canOnlyInteractWithListed),
            Codec.list(ResourceLocation.CODEC).optionalFieldOf("types", new ArrayList<>()).forGetter(o -> o.types.stream().map(Registry.ENTITY_TYPE::getKey).toList()),
        ...
        for (ResourceLocation resourceLocation : typeIds) {
            if (Registry.ENTITY_TYPE.containsKey(resourceLocation)) {
                reinforcements.add(Registry.ENTITY_TYPE.get(resourceLocation));
            }
        }
        for (ResourceLocation resourceLocation : tagIds) {
            reinforcementTags.add(TagKey.create(Registry.ENTITY_TYPE_REGISTRY, resourceLocation));
        }
